...
export default class jsonPrinterResultView extends Component {
    render() {
        const array = this.props.json; //{ result: [...]} or { error: "..." }
        ...
        if (array.error) {
            ...
            rows = array.result.map((val, key) => (
                <View style={style.row} key={lineNumber}>
                    ...
                    <Text style={style.resultContent}>{val.performanceId}   ,      {val.startTime}       ,      {val.endTime}      ,         {val.popularity}</Text>
                </View>
            ))
        }
        return (
